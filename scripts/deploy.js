...
var config = require('yargs')
  .option('s', {
    ...
    type: 'string',
    default: 'dist',
    describe: 'The source directory that you want to publish to the gh-pages branch. ' +
      'This directory will become the root directory of the gh-pages branch.'
  })
  .option('p', {
    alias: 'publishDir',
    type: 'string',
    ...
    describe: 'When publishing to GitHub pages, this directory will be created to publish from. ' +
      'An absolute or relative path can be used. Relative paths will be resolved from the project root.'
  })
  .option('d', {
    ...
    describe: 'If true, the publishDir will be recursively deleted when the script has finished. ' +
      'If you delete the publish directory, performance will be slower because the cache is being deleted.'
  })
  .option('u', {
  ...
  .example('$0 -d -s dist',
    'Publish the contents of the `dist` directory to the gh-pages branch and delete the publish directory afterwards.')
  .help('h')
  .alias('h', 'help')
  .argv;

// Set globals: directories and git remote URL.
...
/**
 * Logs message to console based on verbosity level.
...
/**
 * Deletes the publish directory, default: .publish.
 * To be called after publishing the website.
 * @return
 */
...
(function() {
  ...
  if (!config.allowUncommitted &&
      sh.exec('git diff --exit-code && git diff --staged --exit-code').code !== 0) {
